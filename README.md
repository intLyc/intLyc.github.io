...
- Google Scholar is refreshed at every scheduled `bin/refresh_local.sh` run from a residential IP because Scholar blocks many datacenter addresses. The updater reads the public profile directly with bounded retries and a process-wide hard timeout; a failed attempt preserves the last validated snapshot for the next hourly retry. Incomplete or decreasing snapshots are rejected unless an intentional Scholar profile removal is explicitly confirmed. GitHub Actions handles builds, stars, and visitors, but intentionally does not fetch Scholar.
